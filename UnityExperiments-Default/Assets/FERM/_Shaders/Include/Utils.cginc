...
inline float Q_loop(float Qx, float Qy, float Qz)
{
    return exp(2.302585*(2.5 + .4*Qx + Qy + Qz));
}

inline float Q_minDistance(float Qx, float Qy)
{
    return exp(-2.302585*(3 + Qx));
}

inline float Q_overSample(float Qz)
{
    return exp(-2.302585*Qz);
}

inline float FlatDistance(float3 p, float flatDist)
{
	return length(float2(max(flatDist, 0.0), p.y));
}
